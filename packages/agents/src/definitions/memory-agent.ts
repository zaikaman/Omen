...
import { buildMemorySystemPrompt } from "../prompts/memory/system.js";
...
export const deriveMemoryCheckpoint = (input: z.input<typeof memoryInputSchema>) => {
  ...
  const prompt = buildMemorySystemPrompt({
    ...
    proofCount: parsed.proofArtifacts.length,
  });
  const appendedProofRefs = unique(parsed.proofArtifacts.map((artifact) => artifact.id));
  const noteRefs = parsed.notes.map(
    (note, index) =>
      `note:${index.toString()}:${note.slice(0, 48).trim() || "checkpoint-update"}`,
  );

  return memoryOutputSchema.parse({
    checkpointRefId,
    appendedProofRefs: unique([...appendedProofRefs, ...noteRefs, `prompt:${prompt}`]),
  });
};
