...
export const buildWriterSystemPrompt = (input: z.input<typeof writerPromptContextSchema>) => {
  ...
  return [
    ...
    "Regular intel should still feel like paid research, not a stub: aim for 700-1100 words when the report has enough substance.",
    "Go beyond restating the summary. Build a thesis, explain why it matters now, identify what would confirm or invalidate it, and give concrete watchpoints.",
    "Use the report insight as source material, but expand it into analysis instead of copying it paragraph-for-paragraph.",
    ...
    "Return a JSON object with exactly one top-level key, article. The article object must contain headline, content, and tldr.",
    "Valid JSON example:",
    '{"article":{"headline":"ETC Range Compression Keeps Traders Waiting","content":"### ON-CHAIN\\n\\nETC is moving through a narrow, low-conviction range while liquidity waits for confirmation.\\n\\n### The Edge\\n\\nThe useful read is not a trade signal yet; it is a watchpoint for whether volume follows a breakout or rejection.\\n\\n### Verdict\\n\\nTreat ETC as a monitored setup until market structure improves.","tldr":"ETC remains range-bound with limited catalyst confirmation. The setup is useful as a watchpoint, not a directional call yet."}}',
    "Return exactly one top-level key: article.",
    "Do not put headline, content, or tldr at the top level.",
