...
add column if not exists generated_tweet_text text,
add column if not exists generated_blog_post text,
add column if not exists generator_log_message text,
add column if not exists generator_payload jsonb not null default '{}'::jsonb;
