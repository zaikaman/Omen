...
export const intelSchema = z.object({
  ...
  generatedTweetText: z.string().min(1).max(280).nullable().default(null),
  generatedBlogPost: z.string().min(1).nullable().default(null),
  generatorLogMessage: z.string().min(1).nullable().default(null),
  generatorPayload: z.record(z.string(), z.unknown()).default({}),
