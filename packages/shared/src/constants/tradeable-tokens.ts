export type TradeableTokenCategory =
  | "major"
  | "layer2"
  | "defi"
  | "gaming"
...
export type TradeableToken = {
  symbol: string;
  coingeckoId: string;
  category: TradeableTokenCategory;
...
export const TRADEABLE_TOKENS_MAP = new Map(
  TRADEABLE_TOKENS.map((token) => [token.symbol, token]),
);

export const TRADEABLE_SYMBOLS = TRADEABLE_TOKENS.map((token) => token.symbol);
export const TRADEABLE_SYMBOLS_SET = new Set(TRADEABLE_SYMBOLS);
export const TRADEABLE_TOKENS_COUNT = TRADEABLE_TOKENS.length;

export const SYMBOL_TO_COINGECKO = Object.fromEntries(
  TRADEABLE_TOKENS.map((token) => [token.symbol, token.coingeckoId]),
) as Record<string, string>;

const normalizeLookupSymbol = (symbol: string) =>
  symbol.trim().toUpperCase().replace("USDT", "").replace("-PERP", "");

export const isTradeableSymbol = (symbol: string) =>
  TRADEABLE_SYMBOLS_SET.has(normalizeLookupSymbol(symbol));

export const getTradeableToken = (symbol: string) =>
  TRADEABLE_TOKENS_MAP.get(normalizeLookupSymbol(symbol));

export const getBinanceSymbol = (symbol: string) =>
  getTradeableToken(symbol)?.binanceSymbol;

export const getHyperliquidSymbol = (symbol: string) =>
  getTradeableToken(symbol)?.hyperliquidSymbol;
