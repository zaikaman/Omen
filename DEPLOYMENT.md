...
The node config generated by the entrypoint wires `router_addr` and `a2a_addr` back into those local sidecars automatically.

Important caveat:

- the sidecars are running, but they do not automatically register any custom MCP tools from Omen
- without additional MCP services, the router will be healthy but mostly empty
- the A2A server will still boot and expose an agent card, but the real value comes once you register MCP services behind it
