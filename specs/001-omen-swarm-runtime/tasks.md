...
- [x] T087 [P] [US1] Implement AXL-hosted MCP services for scanner, research, analyst, and critic capabilities in `backend/src/nodes/services/scanner-mcp.ts`, `backend/src/nodes/services/research-mcp.ts`, `backend/src/nodes/services/analyst-mcp.ts`, and `backend/src/nodes/services/critic-mcp.ts`
- [x] T088 [P] [US1] Implement A2A delegation client and response handling for orchestrator-to-specialist workflows in `packages/axl/src/a2a/a2a-client.ts`, `backend/src/nodes/a2a/orchestrator-delegator.ts`, and `backend/src/nodes/a2a/response-correlator.ts`
...
- [x] T051 [P] [US2] Implement dashboard summary aggregation for latest signal, latest intel, system status, next run countdown, and mindshare data in `backend/src/read-models/dashboard-summary.ts` and `backend/src/read-models/runtime-status.ts`
- [x] T052 [P] [US2] Implement analytics snapshot projection for activity, confidence bands, token frequency, win-rate, and mindshare summaries in `backend/src/read-models/analytics-snapshots.ts` and `backend/src/read-models/token-frequency.ts`
...
- [x] T054 [P] [US2] Replace `frontend/src/data/mockData.ts` usage with seeded fallback adapters and live query functions in `frontend/src/lib/api/dashboard.ts`, `frontend/src/lib/api/intel.ts`, `frontend/src/lib/api/signals.ts`, `frontend/src/lib/api/analytics.ts`, and `frontend/src/lib/api/logs.ts`
- [x] T055 [P] [US2] Implement frontend hooks for run status, signals, intel, logs, and analytics in `frontend/src/hooks/useRunStatus.ts`, `frontend/src/hooks/useSignals.ts`, `frontend/src/hooks/useIntel.ts`, `frontend/src/hooks/useLogs.ts`, and `frontend/src/hooks/useAnalytics.ts`
- [x] T056 [US2] Wire `frontend/src/pages/DashboardHome.tsx`, `frontend/src/components/SignalCard.tsx`, `frontend/src/components/IntelCard.tsx`, and `frontend/src/components/TerminalLog.tsx` to live dashboard data and run status responses
- [x] T057 [US2] Wire `frontend/src/pages/SignalsPage.tsx` and `frontend/src/components/ui/SearchAndSort.tsx` to real signal history, filter, sort, and pagination responses
- [x] T058 [US2] Wire `frontend/src/pages/IntelPage.tsx`, `frontend/src/components/IntelBlog.tsx`, and `frontend/src/components/IntelThread.tsx` to live intel feed and detail responses
- [x] T059 [US2] Wire `frontend/src/pages/AnalyticsPage.tsx`, `frontend/src/pages/analytics/AnalyticsOverview.tsx`, `frontend/src/pages/analytics/SignalAnalytics.tsx`, `frontend/src/pages/analytics/MarketAnalytics.tsx`, and `frontend/src/pages/analytics/PerformanceAnalytics.tsx` to live analytics snapshots
...
- [x] T096 [US2] Implement frontend hooks for topology and proof data in `frontend/src/hooks/useTopology.ts` and `frontend/src/hooks/useProofs.ts`
